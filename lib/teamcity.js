...
function Teamcity(runner) {
  ...
  runner.on('suite', function(suite) {
    if (suite.root) return;
	  isfailed = false;
	  log("##teamcity[testStarted name=' " + escape(suite.title) + "']");
  });

  runner.on('test', function(test) {
	  log("test step started: name= " + escape(test.title));
  });

  runner.on('fail', function(test, err) {
    log("test step failed name= " + escape(test.title) + "message= " + escape(err.message));
	  isfailed = true;
  });
